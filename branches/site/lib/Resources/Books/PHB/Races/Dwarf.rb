...
#++
	def apply_level
		...
		if(character.level <= 1)
			...
			character.skill_list.assign_misc("Appraise", 2)
			...
			character.final_levelup_procs += [%{Proc.new do |char|
				if (char.weapon_proficiencies & $MARTIAL_WEAPONS) == $MARTIAL_WEAPONS
					char.weapon_proficiencies |= ["Dwarven Waraxe", "Dwarven Urgrosh"]
				end
			end}]
