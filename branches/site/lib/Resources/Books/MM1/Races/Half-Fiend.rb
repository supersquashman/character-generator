...
#++
	def apply_level
		if(character.level <= 1)
			...
			character.final_levelup_procs += [%{Proc.new {|character| (1..((character.HD/2).floor)).each {|hd| #{@fiend_abilities.to_s}[hd].each {|ability| character.add_ability(ability)}}}}] if (character.stats["int"] >= 8 || character.stats["dex"] >= 8)
			#character.final_levelup_procs += [%{Proc.new {|character|(character.HD/2).floor.times {|hd| character.add_ability(@fiend_abilities[hd])}}}] if (character.stats["int"] >= 8 || character.stats["dex"] >= 8)
			character.final_levelup_procs += [%{Proc.new {|character| character.add_ability("Smite Good (+#{character.HD} damage)")}}]
			character.final_levelup_procs += [%{Proc.new {|character| character.add_ability((character.HD < 12) ? "5/magic" : "10/magic")}}]
			character.final_levelup_procs += [%{Proc.new {|character| character.CR += (character.HD < 5 ? 1 : (character.HD < 11 ? 2 : 3))}}]
		end
